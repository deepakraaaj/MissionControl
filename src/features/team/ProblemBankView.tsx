...
import { AlertOctagon, CheckCircle2, FlaskConical, Plus, Search, Trash2, UserRound } from 'lucide-react';
import { useTeamStore } from './team-store';
import { DiscussButton } from './DiscussButton';
import type { ProblemItem, ProblemSeverity, ProblemStatus } from './team-types';
import { confirmDialog, promptDialog } from '../../components/ui/native-dialog';
...
export function ProblemBankView({ missionId }: ProblemBankViewProps) {
  ...
  const updateProblem = useTeamStore((s) => s.updateProblem);
  ...
  const createProblem = (event: React.FormEvent) => {
    ...
    addProblem({ missionId: target, audienceCategory: audience || 'Customer', title: title.trim(), description: description.trim() || title.trim(), source: source.trim() || 'Team observation', severity, status: 'open', tags: [severity], loggedBy: persona.name, owner: persona.name, evidence: evidence.trim(), impact: impact.trim(), nextAction: nextAction.trim(), occurrenceCount: occurrences });
    setTitle(''); setDescription(''); setSource(''); setEvidence(''); setImpact(''); setNextAction(''); setOccurrences(1); setAdding(false);
  };
  const moveProblem = (problem: ProblemItem, status: ProblemStatus) => {
    if (status === 'solved' && problem.status !== 'solved') {
      void promptDialog('What did we learn or change to solve this issue?', problem.solvedNotes ?? '', { title: 'Record the resolution', confirmLabel: 'Mark solved' }).then((resolution) => {
        if (resolution === null) return;
        updateProblem(problem.id, { solvedNotes: resolution.trim() });
  ...
  const drop = (event: DragEvent<HTMLElement>, status: ProblemStatus) => {
    ...
    if (problem) moveProblem(problem, status);
  ...
  const card = (problem: ProblemItem) => (
    <article key={problem.id} draggable onDragStart={(event) => { setDraggedId(problem.id); event.dataTransfer.setData('text/plain', problem.id); }} onDragEnd={() => { setDraggedId(null); setDropStatus(null); }} className={`group rounded-2xl border border-borderSoft/35 bg-panel p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-borderSoft/60 hover:shadow-md ${draggedId === problem.id ? 'opacity-40' : ''}`}>
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex shrink-0 items-center gap-0.5"><DiscussButton className="opacity-100 lg:opacity-0 lg:group-hover:opacity-100" item={{ kind: 'problem', id: problem.id, label: problem.title, detail: problem.severity }} /><button type="button" aria-label={`Delete ${problem.title}`} onClick={() => { void confirmDialog(`Delete issue “${problem.title}”?`, { title: 'Delete issue', confirmLabel: 'Delete', danger: true }).then((confirmed) => { if (confirmed) deleteProblem(problem.id); }); }} className="rounded-lg p-1.5 text-text-muted opacity-100 hover:bg-rose-500/10 hover:text-rose-500 lg:opacity-0 lg:group-hover:opacity-100"><Trash2 className="h-4 w-4" /></button></div>
      </div>
      ...
      {(problem.impact || problem.evidence) && <div className="mt-3 space-y-2 rounded-xl bg-panel2/45 p-3 text-xs leading-5"><p><span className="font-bold text-text-primary">Impact:</span> <span className="text-text-secondary">{problem.impact || 'Not quantified yet'}</span></p><p><span className="font-bold text-text-primary">Evidence:</span> <span className="text-text-secondary">{problem.evidence || problem.source}</span></p></div>}
      {problem.nextAction && <div className="mt-3 flex items-start gap-2 rounded-xl border border-accent/20 bg-accent/8 p-3"><FlaskConical className="mt-0.5 h-4 w-4 shrink-0 text-accent" /><div><p className="text-[10px] font-bold uppercase tracking-wider text-accent">Next experiment</p><p className="mt-0.5 text-sm leading-5 text-text-primary">{problem.nextAction}</p></div></div>}
      ...
      <div className="mt-4 border-t border-borderSoft/25 pt-3"><div className="flex flex-wrap gap-x-2 gap-y-1 text-[11px] text-text-muted"><strong className="text-text-secondary">{missionName(problem.missionId)}</strong><span>·</span><span className="inline-flex items-center gap-1"><UserRound className="h-3 w-3" />{problem.owner || problem.loggedBy}</span><span>·</span><span>{problem.occurrenceCount ?? 1} signal{(problem.occurrenceCount ?? 1) === 1 ? '' : 's'}</span></div><select value={problem.status} onChange={(event) => moveProblem(problem, event.target.value as ProblemStatus)} className="mt-3 w-full rounded-xl border border-borderSoft/35 bg-panel2/55 px-3 py-2 text-sm text-text-primary lg:hidden">{columns.map((column) => <option key={column.status} value={column.status}>{column.label}</option>)}</select></div>
    </article>
  );

  return <div className="space-y-4">
    <section className="rounded-2xl border border-borderSoft/30 bg-panel/45 p-4">
      <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl border border-rose-500/25 bg-rose-500/10 text-rose-500"><AlertOctagon className="h-5 w-5" /></div><div><h2 className="text-base font-bold text-text-primary">Customer signals</h2><p className="mt-0.5 text-xs text-text-muted">Turn evidence into experiments and reusable learning.</p></div></div><button type="button" disabled={!hasProjects} onClick={() => setAdding(!adding)} title={hasProjects ? 'Capture a signal' : 'Create a project from the Mission tab first'} className="flex items-center gap-2 rounded-xl bg-rose-500 px-4 py-2.5 text-sm font-bold text-white shadow-glow disabled:cursor-not-allowed disabled:opacity-45"><Plus className="h-4 w-4" /> Capture signal</button></div>
    ...
    {adding && <form onSubmit={createProblem} className="grid grid-cols-1 gap-3 rounded-2xl border border-rose-500/30 bg-panel/60 p-4 sm:grid-cols-2 lg:grid-cols-4"><input required value={title} onChange={(e) => setTitle(e.target.value)} placeholder="What did the customer struggle with?" className="rounded-xl border border-borderSoft/35 bg-panel2/55 px-3 py-2.5 text-sm text-text-primary sm:col-span-2" /><select required value={target} onChange={(e) => setTarget(e.target.value)} className="rounded-xl border border-borderSoft/35 bg-panel2/55 px-3 py-2.5 text-sm text-text-primary"><option value="" disabled>Select a project</option>{missions.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select><select value={severity} onChange={(e) => setSeverity(e.target.value as ProblemSeverity)} className="rounded-xl border border-borderSoft/35 bg-panel2/55 px-3 py-2.5 text-sm text-text-primary"><option value="blocker">Blocker — stops adoption</option><option value="friction">Friction — slows users</option><option value="idea">Opportunity — unmet need</option></select><textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Context: when and where does it happen?" className="resize-none rounded-xl border border-borderSoft/35 bg-panel2/55 px-3 py-2.5 text-sm text-text-primary sm:col-span-2" /><textarea value={evidence} onChange={(e) => setEvidence(e.target.value)} placeholder="Evidence: quote, metric, or repeated observation" className="resize-none rounded-xl border border-borderSoft/35 bg-panel2/55 px-3 py-2.5 text-sm text-text-primary sm:col-span-2" /><input value={impact} onChange={(e) => setImpact(e.target.value)} placeholder="Impact: lost revenue, time, trust…" className="rounded-xl border border-borderSoft/35 bg-panel2/55 px-3 py-2.5 text-sm text-text-primary sm:col-span-2" /><input value={nextAction} onChange={(e) => setNextAction(e.target.value)} placeholder="Smallest next experiment" className="rounded-xl border border-borderSoft/35 bg-panel2/55 px-3 py-2.5 text-sm text-text-primary sm:col-span-2" /><input value={audience} onChange={(e) => setAudience(e.target.value)} placeholder="Who experienced it?" className="rounded-xl border border-borderSoft/35 bg-panel2/55 px-3 py-2.5 text-sm text-text-primary" /><input value={source} onChange={(e) => setSource(e.target.value)} placeholder="Source / meeting" className="rounded-xl border border-borderSoft/35 bg-panel2/55 px-3 py-2.5 text-sm text-text-primary" /><label className="flex items-center gap-2 rounded-xl border border-borderSoft/35 bg-panel2/55 px-3 text-xs text-text-secondary">Signals seen<input type="number" min="1" value={occurrences} onChange={(e) => setOccurrences(Math.max(1, Number(e.target.value)))} className="min-w-0 flex-1 bg-transparent py-2.5 text-right text-sm text-text-primary outline-none" /></label><div className="flex justify-end gap-2 sm:col-span-2 lg:col-span-3"><button type="button" onClick={() => setAdding(false)} className="px-4 py-2 text-sm text-text-muted">Cancel</button><button type="submit" disabled={!target} className="rounded-xl bg-rose-500 px-4 py-2 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-45">Capture signal</button></div></form>}
