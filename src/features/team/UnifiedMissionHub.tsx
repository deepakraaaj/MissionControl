...
type StrategyDraft = Pick<TeamMissionItem, 'description' | 'why_it_matters' | 'objective' | 'definition_of_success' | 'customer_segment' | 'revenue_model'>;

function ProjectOverview({ mission, tasks, leads, problems, onUpdate, syncStatus, syncError }: {
  mission: Mission | TeamMissionItem;
  tasks: TeamTask[];
  leads: Lead[];
  problems: ProblemItem[];
  onUpdate?: (updates: StrategyDraft) => void;
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-4">
      <section className="overflow-hidden rounded-2xl border border-borderSoft/60 bg-panel">
        <div className="flex flex-col gap-4 p-4 sm:flex-row sm:items-start sm:justify-between sm:p-6">
          <div className="min-w-0">
            ...
            <h1 className="text-xl font-bold tracking-tight text-text-primary sm:text-2xl">{mission.title}</h1>
            <p className="mt-1 max-w-3xl text-sm leading-6 text-text-secondary">{mission.description}</p>
          </div>
          <div className="flex shrink-0 items-center gap-2">
            {onUpdate && <button type="button" onClick={beginEditing} className="flex h-9 items-center gap-2 rounded-xl border border-borderSoft/60 bg-panel2/60 px-3 text-xs font-semibold text-text-primary transition-colors hover:border-accent/40 hover:text-accent"><Pencil className="h-3.5 w-3.5" />Edit summary</button>}
      ...
      {editing && (
        <form onSubmit={saveStrategy} className="rounded-2xl border border-accent/30 bg-panel p-4 shadow-sm sm:p-5">
          <div className="mb-4 flex items-start justify-between gap-3"><div><h2 className="text-base font-bold text-text-primary">Edit project summary</h2><p className="mt-0.5 text-xs text-text-secondary">Changes sync to the shared Supabase team room.</p></div><button type="button" onClick={() => setEditing(false)} className="text-xs font-medium text-text-secondary hover:text-text-primary">Cancel</button></div>
          <div className="grid gap-3 lg:grid-cols-2">
            {([
              ['description', 'One-line summary'], ['customer_segment', 'Target customer'],
              ['why_it_matters', 'Problem statement'], ['objective', 'How we solve it'],
              ['revenue_model', 'Revenue model'], ['definition_of_success', 'Definition of success'],
            ] as const).map(([key, label]) => (
              <label key={key} className="block text-xs font-semibold text-text-secondary">{label}
                <textarea value={draft[key] ?? ''} onChange={(event) => setDraft((current) => ({ ...current, [key]: event.target.value }))} rows={3} className="mt-1.5 w-full resize-y rounded-xl border border-borderSoft bg-panel2/50 px-3 py-2.5 text-sm font-normal leading-5 text-text-primary outline-none transition-colors placeholder:text-text-muted focus:border-accent" />
              </label>
            ))}
          </div>
          <div className="mt-4 flex justify-end"><button type="submit" className="flex h-10 items-center gap-2 rounded-xl bg-accent px-4 text-sm font-semibold text-[rgb(var(--accent-contrast))]"><Save className="h-4 w-4" />Save to backend</button></div>
        </form>
      )}
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <article className="rounded-2xl border border-borderSoft/60 bg-panel p-4 sm:p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-text-primary">{teamMission.customer_segment || 'Not defined yet. Edit the summary to add it.'}</p>
        </article>
        <article className="rounded-2xl border border-borderSoft/60 bg-panel p-4 sm:p-5">
          <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-danger">Problem statement</p>
          <p className="mt-2 text-sm leading-6 text-text-primary">{mission.why_it_matters || mission.description}</p>
        </article>
        <article className="rounded-2xl border border-borderSoft/60 bg-panel p-4 sm:p-5">
          <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-accent">How we solve it</p>
          <p className="mt-2 text-sm leading-6 text-text-primary">{mission.objective || mission.description}</p>
        </article>
        <article className="rounded-2xl border border-borderSoft/60 bg-panel p-4 sm:p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-text-primary">{teamMission.revenue_model || 'Not defined yet. Edit the summary to add it.'}</p>
        </article>
      </section>
      ...
      <section className="grid gap-3 lg:grid-cols-2">
        <article className="rounded-2xl border border-borderSoft/60 bg-panel p-4 sm:p-5">
          ...
          <p className="mt-2 text-sm font-medium leading-6 text-text-primary">{mission.definition_of_success || 'Define a measurable success outcome for this project.'}</p>
        </article>
        <article className="rounded-2xl border border-borderSoft/60 bg-panel p-4 sm:p-5">
          ...
          <div className="mt-2 space-y-2">
            {nextTasks.length ? nextTasks.map((task) => (
              <div key={task.id} className="flex items-start gap-2 text-sm text-text-primary"><CheckSquare className="mt-0.5 h-4 w-4 shrink-0 text-accent" /><span className="leading-5">{task.title}</span></div>
