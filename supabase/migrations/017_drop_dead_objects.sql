...
-- ──────────────────────────────────────────
-- 1-2. Rescue anything still living only in the legacy blob, prove nothing is
-- stranded, then drop it.
--
-- Wrapped in an existence check so the migration is safe to run twice — once by
-- hand in the SQL editor and again via `db push`, for instance. Without it the
-- second run would fail on a missing team_room_state rather than doing nothing.
DO $outer$
DECLARE
  stranded text;
BEGIN
  IF to_regclass('public.team_room_state') IS NULL THEN
    RAISE NOTICE 'team_room_state already dropped; nothing to migrate.';
  ELSE
    --
    ...
    DROP TABLE public.team_room_state;
  END IF;
END $outer$;
